...
Pulled up the existing workflow for the kiosk system
Mapped where the gaps were
Discussed stakeholder needs in context of the existing workflow
Designed a solution that expanded the workflow rather than bypassing it
Implemented a scalable solution, not a one-off fix
