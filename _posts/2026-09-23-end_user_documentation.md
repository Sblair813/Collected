...
1. Description paragraph. Every process opened with a short paragraph explaining the feature or functionality: what it is, why it matters, and when the user would need it. This gave users context before they touched anything, so the steps that followed had meaning rather than feeling like a list of clicks to memorize.

2. Read-through instructions. Next came a narrative walkthrough of the process from start to finish. Users could read it before attempting the task to understand the overall flow, what to expect along the way, and how each part connected to the next. It built a mental map, so nothing in the steps came as a surprise.

3. Step-by-step instructions. Finally, users got clear, numbered steps with images to follow while performing the task. Each step contained a single action, written consistently from one manual to the next, so users always knew where to look and what to do.
